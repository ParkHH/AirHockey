...
사용법 : getRandom 호출시 당신이 원하는 수를 넣으세요.
	ex) getRandom(5) 를 넣으면 0~4를 반환 받습니다.
--------------------------------------------------------*/
function getRandom(arr){
	var y = Math.random()
	var x = parseInt(y*arr);
	// menu 배열의 길이 또는 지정 정수를 Math.random() 에 곱해주면 (0.9)*3=2.7 이 최대의 값이라고 생각할 수 있다.
	// 결과값을 정수화 시킨다면 2이고 이것은 menu 배열의 길이와 같다.

	return x;
	// 반환형을 갖는 함수, return 을 만나면 자신을 호출한 부분으로 지정된 값을 들고 나간다.
}
...
인수로 넘긴 n 이 10보다 작으면, 숫자 앞에 '0' 을 조합해서 결과를 반환
--------------------------------------------------------------------*/
...
function getLastDateOfMonth(year, month){
	...
	//조작하기
	d.setFullYear(year);
	d.setMonth(month+1);//자바스크립트에서는 마지막날을 구할 수 없기때문에 정보를 다음달로 넘어간 후 0일을 구한다.
	d.setDate(0);
